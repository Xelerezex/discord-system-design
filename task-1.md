...
- Запросов в день:
  `Req_read = DAU × C_open = 10,000,000 X 10 = 100,000,000 /day`
- Средний RPS:
...
- Сообщений в день:
  `Msg_day = DAU_comm × M_per_comm = 3,000,000 X 15 = 45,000,000 /day`
- Средний RPS:
...
#### D) Read-states (User×Channel) - горячая точка
...
1–3) **Gateway (WebSocket) ×3** - держат постоянные подключения клиентов
4–5) **API (HTTP/gRPC) ×2** - auth, guild/channel APIs
6–7) **Message Service ×2** - запись/чтение сообщений, coalescing, rate limit
8) **Read-States Service ×1** - отдельный hot-path сервис
9) **Search/Index Service ×1** - индексирование и поиск
10) **Storage/DB nodes ×N** - минимум 1-2 для учебного стенда
