...
- [Высоконагруженный мессенджер-клон Telegram](#высоконагруженный-мессенджер-клон-telegram)
	...
	- [Основная часть](#основная-часть)
		...
		- [11. Список серверов](#11-список-серверов)
			...
			- [Оценка стоимости инфраструктуры](#оценка-стоимости-инфраструктуры)
...
| --------------------------------- | ----------------------------------------------------------------------: | --------------------------------------------------------------------------------------------------- | -------- |
| `ingress-nginx` / HTTPS RPS       |                `342 785 HTTPS RPS` на `24 CPU`; throughput `8.8 Gbit/s` | Расчёт обычного HTTPS/WebSocket proxy-path для `Ingress L7` из схемы проекта                        | [^77]    |
| `ingress-nginx` / TLS termination |                                          `10 274 HTTPS CPS` на `24 CPU` | Консервативный расчёт TLS termination для входного `Ingress L7` из схемы проекта                    | [^78]    |
| C++ лёгкое JSON API               |                                `5000 RPS / CPU core`; `10 MB RAM / CPU` | `auth-service`, `updates-api`                                                                       | [^85]    |
| C++ средняя бизнес-логика         |                                `100 RPS / CPU core`; `100 MB RAM / CPU` | `api-message-service`, `dialogs-api`, `fanout-projector`, `push-service`, processing в `ws-gateway` | [^85]    |
| WebSocket memory                  |           `64 KiB` на соединение по умолчанию; `14 KiB` без compression | Базовая RAM только на поддержание открытых соединений в `ws-gateway`                                | [^79]    |
...
| Сетевые карты серверов            |   базовый worker-узел: `2×25 Gbit/s`; observability-узел: `2×10 Gbit/s` | Отдельный расчёт сетевой ёмкости в итоговой таблице серверов                                        | [^85]    |
#### Расчёт ресурсов по сервисам
| Сервис                |                                      Пиковая нагрузка | Расчёт                                                                                                                                                                                        |                                 Итоговые ресурсы | Источник    |
| --------------------- | ----------------------------------------------------: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -----------------------------------------------: | ----------- |
| `ingress-nginx`       |                    `~53 885 RPS` + TLS/WebSocket вход | HTTPS RPS: `53 885 / (342 785 / 24) × 1.5 ≈ 6 CPU`;TLS/CPS: `53 885 / (10 274 / 24) × 1.5 ≈ 189 CPU`;Net: `1.62 / 8.8 ≈ 1`;узкое место — TLS termination                                      |             `192 CPU`, `48 GB RAM`, `≥25 Gbit/s` | [^77] [^78] |
| `ws-gateway`          |      `~1.96 млн WS-соединений`; доставка `23 889 RPS` | CPU: `23 889 / 100 × 1.5 ≈ 359`;RAM baseline: `1.96M × 64 KiB ≈ 119.6 GiB`;RAM processing: `360 × 100 MB ≈ 33.5 GiB`;RAM с запасом: `(119.6 + 33.5) × 1.5 ≈ 229.7 GiB`, округляем до `384 GB` |            `360 CPU`, `384 GB RAM`, `≥10 Gbit/s` | [^79] [^85] |
...
| ------------------------ | --------------------- | -----------------------------------: | --------------------------------------: | -----------: | ----------------: | ------------------------------------------------ | ------------------------------------------------------------------------------- | ----------------- |
| Kubernetes control plane | Managed Kubernetes    |                     HA control plane |                                     `1` |            — |                 — | Managed-сеть, вне расчёта проекта                | Kubernetes API / scheduler / controller-manager                                 | [^84]             |
| Edge / WS pool           | Bare-metal Kubernetes | `32 CPU / 128 GB RAM / NVMe / 2×25G` |                                    `18` |    `576 CPU` |         `2304 GB` | `18 × 2 × 25 = 900 Gbit/s`                       | `ingress-nginx`, `ws-gateway`                                                   | [^77] [^78] [^79] |
| Backend pool             | Bare-metal Kubernetes | `32 CPU / 128 GB RAM / NVMe / 2×25G` |                                   `160` |   `5120 CPU` |        `20480 GB` | `160 × 2 × 25 = 8000 Gbit/s`                     | `auth`, `api-message`, `updates`, `dialogs`, `fanout`, `push`, `pgbouncer`      | [^85]             |
| Kafka brokers            | Bare-metal            |  `16 CPU / 64 GB RAM / NVMe / 2×25G` |                                     `6` |     `96 CPU` |          `384 GB` | `6 × 2 × 25 = 300 Gbit/s`                        | Очереди `message-created`, `fanout-user-update`, `push-notification`, retry/DLQ | [^80]             |
| PostgreSQL HA            | Bare-metal            | `32 CPU / 128 GB RAM / NVMe / 2×25G` |                                     `6` |    `192 CPU` |          `768 GB` | `6 × 2 × 25 = 300 Gbit/s`                        | `users`, `user_sessions`, `secret_chats`, Patroni HA                            | [^83]             |
| etcd для Patroni         | VM / small bare-metal |        `4 CPU / 8 GB RAM / SSD / 1G` |                                     `3` |     `12 CPU` |           `24 GB` | `3 × 1 = 3 Gbit/s`                               | Quorum для Patroni                                                              | [^83]             |
| Citus coordinators       | Bare-metal            | `32 CPU / 128 GB RAM / NVMe / 2×25G` |                                     `3` |     `96 CPU` |          `384 GB` | `3 × 2 × 25 = 150 Gbit/s`                        | Citus coordinator HA                                                            | [^83]             |
| Citus workers            | Bare-metal            | `32 CPU / 256 GB RAM / NVMe / 2×25G` |                                    `24` |    `768 CPU` |         `6144 GB` | `24 × 2 × 25 = 1200 Gbit/s`                      | `messages`, `chat_members`, `user_updates`, `dialogs_by_user`                   | [^81]             |
| Redis presence           | Bare-metal            |  `16 CPU / 64 GB RAM / NVMe / 2×25G` |                                     `6` |     `96 CPU` |          `384 GB` | `6 × 2 × 25 = 300 Gbit/s`                        | `user_presence`, online/offline, last seen                                      | [^82]             |
| Observability pool       | Bare-metal Kubernetes |  `16 CPU / 64 GB RAM / NVMe / 2×10G` |                                     `6` |     `96 CPU` |          `384 GB` | `6 × 2 × 10 = 120 Gbit/s`                        | Prometheus, Loki, Grafana, Jaeger, Alertmanager                                 | [^84]             |
| **Итого**                | —                     |                                    — | **232 сервера + managed control plane** | **7052 CPU** | **31 256 GB RAM** | **446 NIC по 25G + 12 NIC по 10G + 3 NIC по 1G** | —                                                                               | —                 |

#### Оценка стоимости инфраструктуры

Расчёт сделан по публичным ценам Selectel на выделенные серверы и Managed Kubernetes. Цены Selectel указаны с НДС 22%. Для dedicated-серверов в стоимость входит базовая сеть: безлимитный трафик на `1 Гбит/с` или `300 ТБ/мес.` с возможностью расширения канала, приватная сеть до `25 Гбит/с`, публичный IPv4, DDoS-защита и KVM/IPMI-инфраструктура [^86] [^87].
...
| -------------------------------- | --------------: | ------------------------------------ | ------------------------------------------------------------- | --------------------: | ------------------------: | ---------------------------------------------------------------------------------------------------- |
| Managed Kubernetes control plane |             `1` | HA control plane                     | Кластер Managed Kubernetes, отказоустойчивый, `3 master-ноды` |         `15 532,36 ₽` |             `15 532,36 ₽` | Цена HA-кластера Kubernetes по прайсу Selectel.                                                      |
| Edge / WS pool                   |            `18` | `32 CPU / 128 GB RAM / NVMe / 2×25G` | `AL81-NVMe-10GE`: `32 ядра`, `128 GB RAM`, `2×960 GB NVMe`    |            `58 300 ₽` |             `1 049 400 ₽` | Ближайшее совпадение по CPU/RAM/NVMe; точная публичная цена кастомного `2×25G` не выделена отдельно. |
| Backend pool                     |           `160` | `32 CPU / 128 GB RAM / NVMe / 2×25G` | `AL81-NVMe-10GE`: `32 ядра`, `128 GB RAM`, `2×960 GB NVMe`    |            `58 300 ₽` |             `9 328 000 ₽` | Основной пул backend-сервисов.                                                                       |
| Kafka brokers                    |             `6` | `16 CPU / 64 GB RAM / NVMe / 2×25G`  | `DL-37`: `24 ядра`, `64 GB RAM`, `2×960 GB NVMe`              |            `29 000 ₽` |               `174 000 ₽` | Взята ближайшая конфигурация с запасом по CPU и NVMe.                                                |
| PostgreSQL HA                    |             `6` | `32 CPU / 128 GB RAM / NVMe / 2×25G` | `AL81-NVMe-10GE`: `32 ядра`, `128 GB RAM`, `2×960 GB NVMe`    |            `58 300 ₽` |               `349 800 ₽` | HA-контур PostgreSQL: `2 × (1 primary + 2 standby)`.                                                 |
| etcd для Patroni                 |             `3` | `4 CPU / 8 GB RAM / SSD`             | Cloud VM: `4 vCPU + 8 GB RAM + 100 GB local SSD`              |         `≈5 757,72 ₽` |            `≈17 273,16 ₽` | Расчёт: `4×668,35 + 8×243,04 + 100×11,40`.                                                           |
| Citus coordinators               |             `3` | `32 CPU / 128 GB RAM / NVMe / 2×25G` | `AL81-NVMe-10GE`: `32 ядра`, `128 GB RAM`, `2×960 GB NVMe`    |            `58 300 ₽` |               `174 900 ₽` | Координаторы Citus.                                                                                  |
| Citus workers                    |            `24` | `32 CPU / 256 GB RAM / NVMe / 2×25G` | `AL63-NVMe`: `32 ядра`, `256 GB RAM`, `2×960 GB NVMe`         |            `71 800 ₽` |             `1 723 200 ₽` | Ближайшее совпадение по CPU/RAM/NVMe.                                                                |
| Redis presence                   |             `6` | `16 CPU / 64 GB RAM / NVMe / 2×25G`  | `DL-37`: `24 ядра`, `64 GB RAM`, `2×960 GB NVMe`              |            `29 000 ₽` |               `174 000 ₽` | Взята ближайшая конфигурация с запасом по CPU.                                                       |
| Observability pool               |             `6` | `16 CPU / 64 GB RAM / NVMe / 2×10G`  | `DL-37`: `24 ядра`, `64 GB RAM`, `2×960 GB NVMe`              |            `29 000 ₽` |               `174 000 ₽` | Prometheus, Loki, Grafana, Jaeger, Alertmanager.                                                     |
| **Итого**                        | **233 позиции** | —                                    | —                                                             |                     — | **≈13 180 105,52 ₽/мес.** | **≈158 161 266,24 ₽/год**                                                                            |
...
[^86]: [Аренда выделенного сервиса Selectel](https://selectel.ru/services/dedicated/)

[^87]: [Тарифы на услуги Selectel](https://selectel.ru/prices/)
