...
| Домен                       | Назначение                                                                     | Отношение к ЦОД                                                                                                                     |
| --------------------------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- |
| Идентификация и сессии      | Регистрация/авторизация, JWT/refresh, управление сессиями                      | **Только ЦОД** (персональные данные, консистентность сессий).                                                                       |
| Гильдии/каналы (метаданные) | Список серверов пользователя, список каналов, членства, ACL/роли               | **ЦОД** (источник прав/ACL), чтение можно кэшировать на API-уровне; запись/изменения - строго в ЦОД                                 |
| Сообщения (write path)      | `POST /channels/{id}/messages`, валидация, запись сообщения, генерация событий | **Только ЦОД** (низкая задержка записи и доставка событий из ближайшей площадки)                                                    |
| Сообщения (read path)       | `GET /channels/{id}/messages`, пагинация истории                               | **ЦОД** (источник), чтение частично ускоряется кэшированием                                                                         |
| Реалтайм доставка           | WebSocket подключения, подписки на каналы, доставка сообщений, presence        | **ЦОД** (долгие соединения), маршрутизация в ближайший ЦОД                                                                          |
| Read/unread и упоминания    | Статусы прочтения, счетчики непрочитанного, mentions                           | **ЦОД** (источник), агрегация/кэш счетчиков на площадке; меж-ЦОД синк асинхронный                                                   |
| Модерация и роли            | Бан/мут, управление ролями, настройка прав                                     | **Только ЦОД** (операции, критичные к целостности ACL и аудит-логам)                                                                |
| Поиск по сообщениям         | Индексация, поиск по истории                                                   | **ЦОД** (search-кластер рядом с данными); допускается задержка индексации (eventual consistency) и асинхронный репликатор между ЦОД |
|                             |                                                                                |                                                                                                                                     |
...
| ------------------------------------ | ----------------------------------------------------------------------------------- |
| Время доставки сообщения             | Ближайший ЦОД -> меньше RTT для `POST` и быстрее “сообщение дошло”.                 |
