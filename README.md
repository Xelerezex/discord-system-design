...
- [Высоконагруженный мессенджер-клон Telegram](#высоконагруженный-мессенджер-клон-telegram)
	...
	- [Основная часть](#основная-часть)
		...
		- [5. Логическая схема БД](#5-логическая-схема-бд)
			...
			- [Размеры данных и нагрузки на чтение/запись](#размеры-данных-и-нагрузки-на-чтение/запись)
...
| **Сущность данных** | **Размер объекта avg** | **Суточный поток данных, ГБ/сут** | **Запись, QPS avg/peak** | **Чтение, QPS avg/peak** | **Основание**                                                                                       |
| ------------------- | ---------------------: | --------------------------------: | -----------------------: | -----------------------: | --------------------------------------------------------------------------------------------------- |
| **storage_files**   |                  128 Б |                              0.06 |                   5 / 20 |     4,180.56 / 16,722.22 | Запись/чтение файловых метаданных; для MVP принята проектная оценка 5 загрузок/с.                   |
| **users**           |                  256 Б |                              0.02 |                    1 / 4 |     4,180.56 / 16,722.22 | Запись профиля - редкое проектное допущение. Чтение - загрузка профиля на сессию.                   |
| **user_sessions**   |                  256 Б |                             92.47 |     4,180.56 / 16,722.22 |    19,443.57 / 77,774.28 | Новая/обновленная сессия на каждый session/reconnect. Чтение - auth check на send + sync + session. |
| **messages**        |                  640 Б |                            513.74 |     9,290.79 / 37,163.15 |   59,722.22 / 238,888.89 | Запись - 1 строка на 1 отправку. Чтение - выдача сообщений при доставке/sync.                       |
| **presence_cache**  |                   64 Б |                             23.12 |     4,180.56 / 16,722.22 |   59,722.22 / 238,888.89 | Запись - обновление статуса при session/reconnect. Чтение - lookup online/offline при fanout.       |
| **chats**           |                  160 Б |                            128.44 |     9,290.79 / 37,163.15 |    15,263.01 / 61,052.04 | Запись - обновление `last_message_id` и `updated_at` на каждую отправку. Чтение - send + sync       |
| **chat_members**    |                   80 Б |                            412.80 |   59,722.22 / 238,888.89 |    15,263.01 / 61,052.04 | Запись - обновление unread/read-state по каждому получателю. Чтение - ACL check + sync state        |
| **update_buffer**   |                  160 Б |                            825.60 |   59,722.22 / 238,888.89 |   59,722.22 / 238,888.89 | На каждого получателя создается буферная запись и затем читается при sync                           |
| **event_log**       |                  160 Б |                          1,011.83 |   73,193.56 / 292,774.26 |                    0 / 0 | Запись - send + delivery + session events. Online-чтение не учитывается                             |
