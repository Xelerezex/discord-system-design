...
Для получения картинки использовался сервис [dbdiagram](https://dbdiagram.io/) со кодом из [этого файла](resources/database.sql) ([интерактивная ссылка на работу](https://dbdiagram.io/d/69b26723cf54053b6f6b39fe)). 
...
| ------------------ | --------------------------------------------- | ------------------------- |
| **storage_files**  | Метаданные файлов                             | Постоянные метаданные     |
| **users**          | Профиль пользователя                          | Постоянные                |
| **user_sessions**  | Авторизация и активные сессии                 | Временные                 |
| **messages**       | История сообщений                             | Постоянные                |
| **presence_cache** | Онлайн-статус и last seen                     | Временные                 |
| **chats**          | Метаданные диалогов, групп и каналов          | Постоянные                |
| **chat_members**   | Участники чатов и их состояние                | Постоянные                |
| **update_buffer**  | Буфер обновлений для доставки и синхронизации | Временные                 |
| **event_log**      | Журнал пользовательских и системных событий   | Потоковые / аналитические |

#### Размеры данных и нагрузки на чтение/запись

| **Сущность данных** | **Характер хранения** | **Средний размер строки / объекта** | **Суточный поток данных, ГБ/сут** | **Запись, QPS avg ** | **Запись, QPS peak** | **Чтение, QPS avg** | **Чтение, QPS peak** | **Основание**                                                                                       |
| ------------------- | --------------------- | ----------------------------------: | --------------------------------: | -------------------: | -------------------- | ------------------: | -------------------- | --------------------------------------------------------------------------------------------------- |
| **storage_files**   | Постоянные            |                               128 Б |                              0.06 |                    5 | 20                   |            4,180.56 | 16,722.22            | Запись/чтение файловых метаданных; для MVP принята проектная оценка 5 загрузок/с.                   |
| **users**           | Постоянные            |                               256 Б |                              0.02 |                    1 | 4                    |            4,180.56 | 16,722.22            | Запись профиля - редкое проектное допущение. Чтение - загрузка профиля на сессию.                   |
| **user_sessions**   | Временные             |                               256 Б |                             92.47 |             4,180.56 | 16,722.22            |           19,443.57 | 77,774.28            | Новая/обновленная сессия на каждый session/reconnect. Чтение - auth check на send + sync + session. |
| **messages**        | Постоянные            |                               640 Б |                            513.74 |             9,290.79 | 37,163.15            |           59,722.22 | 238,888.89           | Запись - 1 строка на 1 отправку. Чтение - выдача сообщений при доставке/sync.                       |
| **presence_cache**  | Временные             |                                64 Б |                             23.12 |             4,180.56 | 16,722.22            |           59,722.22 | 238,888.89           | Запись - обновление статуса при session/reconnect. Чтение - lookup online/offline при fanout.       |
| **chats**           | Постоянные            |                               160 Б |                            128.44 |             9,290.79 | 37,163.15            |           15,263.01 | 61,052.04            | Запись - обновление `last_message_id` и `updated_at` на каждую отправку. Чтение - send + sync       |
| **chat_members**    | Постоянные            |                                80 Б |                            412.80 |            59,722.22 | 238,888.89           |           15,263.01 | 61,052.04            | Запись - обновление unread/read-state по каждому получателю. Чтение - ACL check + sync state        |
| **update_buffer**   | Временные             |                               160 Б |                            825.60 |            59,722.22 | 238,888.89           |           59,722.22 | 238,888.89           | На каждого получателя создается буферная запись и затем читается при sync                           |
| **event_log**       | Потоковые             |                               160 Б |                          1,011.83 |            73,193.56 | 292,774.26           |                  0* | 0*                   | Запись - send + delivery + session events. Online-чтение не учитывается                             |

Все расчеты из этой таблицы представлены в [файле](archive/database-calculation).

#### Требования к консистентности

| **Таблица / связка**                      | **Что должно быть согласовано**                                  | **Требование** | **Основание**                                                  |
| ----------------------------------------- | ---------------------------------------------------------------- | -------------- | -------------------------------------------------------------- |
| **messages + chats.last_message_id**      | Запись нового сообщения и обновление последнего сообщения чата.  | **Strong**     | Нельзя показывать новый message без обновления состояния чата. |
| **chat_members при отправке**             | Проверка участия пользователя в чате и его роли.                 | **Strong**     | Исключенный пользователь не должен писать в чат.               |
| **chat_members.last_read_seq_no**         | Монотонное продвижение read pointer.                             | **Strong**     | Иначе ломается unread_count и read-state.                      |
| **user_sessions**                         | Выдача, продление и инвалидирование токена                       | **Strong**     | Нельзя признавать недействительную сессию валидной.            |
| **storage_files + ссылка из users/chats** | Ссылка на файл появляется только после успешной загрузки объекта | **Strong**     | Нельзя хранить битую ссылку на отсутствующий объект.           |
| **update_buffer после commit в messages** | Обновление может появиться позже самого сообщения.               | **Eventual**   | Сообщение уже надежно сохранено, доставка может догонять.      |
| **presence_cache**                        | Online/offline и last_seen обновляются с задержкой.              | **Eventual**   | Небольшая задержка статуса для мессенджера допустима.          |
| **event_log**                             | Запись события относительно пользовательского ответа.            | **Eventual**   | Событие может быть долито асинхронно после основной операции   |

#### Особенности распределения нагрузки по ключам
| **Таблица**        | **Основной ключ нагрузки**         | **Как распределяется нагрузка**                                               | **hot key / особенности**                                            |
| ------------------ | ---------------------------------- | ----------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| **storage_files**  | `id`, `object_key`                 | Метаданные распределены ровно, чтения концентрируются по популярным объектам. | Общий аватар/фото канала может стать горячим объектом.               |
| **users**          | `id`, `username`, `phone_number`   | Распределение в целом равномерное.                                            | Горячими бывают популярные аккаунты, но это не основное узкое место. |
| **user_sessions**  | `user_id`, `token`                 | Нагрузка тяготеет к активным пользователям и multi-device сценариям.          | Пользователи с несколькими устройствами и частыми переподключениями. |
| **messages**       | `(chat_id, seq_no)`                | Основная - `chat_id`.                                                         | Самые активные чаты становятся горячими партициями.                  |
| **presence_cache** | `(user_id, device_id)`             | Нагрузка user-centric.                                                        | Горячие пользователи с большим числом устройств.                     |
| **chats**          | `id`                               | Запись концентрируется по активным чатам.                                     | Крупные группы и каналы становятся hot key.                          |
| **chat_members**   | `(chat_id, user_id)`               | Запись делится между chat-centric и user-centric доступом.                    | Большие каналы дают всплеск при многих участникам.                   |
| **update_buffer**  | `user_id`                          | Fanout в итоге разносит нагрузку по получателям.                              | Массовые каналы создают всплеск по множеству `user_id`.              |
| **event_log**      | `created_at`, `user_id`, `chat_id` | Поток append-only, естественная ось - время.                                  | Hot не по одному ключу, а по окнам времени.                          |
